...
| `npm run web` | 在电脑浏览器里打开网页版（开发模式，改代码自动刷新） |
...
网址：**https://englishapp-cloud.github.io/English_Reading/**
...
- 朗读用浏览器自带的声音，不同浏览器听起来不一样，有的浏览器可能没有声音。
- 进度存在对方那个浏览器里，换浏览器或清缓存就没了。
- 网页版是正式版，设置页没有开发工具（模拟日期等），所以复习要等收藏后第二天才会出现。
- `github.io` 在国内有时候打开慢，偶尔打不开，换个网络或过一会儿再试。
